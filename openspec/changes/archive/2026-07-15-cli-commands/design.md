...
**6. All four files live under `commands/dev-flow/`, and MAY carry a normal `description:` frontmatter field like any other command in this repo — that field does not cause auto-triggering.**
Earlier drafting of this design assumed `description:` itself was the auto-trigger marker; it isn't. This repo's own existing command files (e.g. `.claude/commands/opsx/propose.md`) already have a `description:` field used for `/help` listing, and are not auto-triggered. Claude Code's auto-trigger mechanism is keyed off a file being a `SKILL.md` under a `skills/` directory, not off the presence of any particular frontmatter field on a command. So these four files should follow the same convention as other commands in this repo (a short `description:` for discoverability) — the actual invariant that matters is that none of them live under a `skills/` directory, which would make them semantically matchable and auto-triggered the way `skills/dev-flow/SKILL.md` is.
